                        ...
                        @foreach($posts as $post)
                            <article class="card-hover group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-neutral-100 transition-all duration-300 hover:shadow-lg">
                                ...
                                <div class="p-6">
                                    ...
                                    <p class="mt-3 line-clamp-3 text-sm leading-relaxed text-neutral-600">{{ Str::limit($post->excerpt, 150) }}</p>
                                    <div class="mt-4 flex items-center justify-between border-t border-neutral-100 pt-4">
                                        <div class="flex items-center gap-3">
                                            <img src="{{ asset($post->author->avatar ?? 'images/team-placeholder.jpg') }}" alt="{{ $post->author->name }}" class="h-8 w-8 rounded-full object-cover">
                                            <span class="text-sm font-medium text-neutral-700">{{ $post->author->name }}</span>
                                        </div>
                                        @if($post->tags->count() > 0)
                                            <div class="flex gap-1">
                                                @foreach($post->tags->take(2) as $tag)
                                                    <span class="rounded bg-neutral-100 px-2 py-0.5 text-xs text-neutral-500">#{{ $tag->name }}</span>
                                                @endforeach
                                            </div>
                                        @endif
                                    </div>
