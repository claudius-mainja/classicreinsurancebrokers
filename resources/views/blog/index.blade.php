                            ...
                            <article class="fade-up card-glass overflow-hidden rounded-xl border border-neutral-800 bg-neutral-900/60 backdrop-blur-sm transition-all duration-300 hover:border-primary-600/30 group">
                                ...
                                <div class="p-6">
                                    <div class="mb-3 flex flex-wrap items-center gap-2">
                                        ...
                                        <span class="text-xs text-neutral-500">{{ $post->published_at->format('M d, Y') }}</span>
                                    </div>
                                    <h2 class="text-lg font-bold text-white transition-colors group-hover:text-primary-600"><a href="{{ route('blog.show', $post->slug) }}">{{ $post->title }}</a></h2>
                                    <p class="mt-2 text-sm leading-relaxed text-neutral-400 line-clamp-3">{{ Str::limit($post->excerpt, 150) }}</p>
                                    <div class="mt-4 flex items-center justify-between border-t border-neutral-800 pt-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-sm font-medium text-neutral-300">{{ $post->author->name }}</span>
                                        </div>
                                        @if($post->tags->count() > 0)
                                            <div class="flex gap-1">
                                                @foreach($post->tags->take(2) as $tag)<span class="rounded bg-neutral-800 px-2 py-0.5 text-xs text-neutral-400">#{{ $tag->name }}</span>@endforeach
                                            </div>
                                        @endif
                                    </div>
                                </div>
                ...
                <div class="sticky top-24 space-y-6">
                    <div class="fade-up card-glass rounded-xl border border-neutral-800 bg-neutral-900/60 backdrop-blur-sm p-6">
                        ...
                        <ul class="mt-4 space-y-1">
                            @foreach($categories ?? [] as $category)
                                <li><a href="{{ route('blog.category', $category->slug) }}" class="flex items-center justify-between rounded-lg px-3 py-2 text-sm text-neutral-400 transition-colors hover:bg-neutral-800 hover:text-primary-600 {{ request('category') === $category->slug ? 'bg-neutral-800 text-primary-600 font-medium' : '' }}">{{ $category->name }} <span class="rounded-md bg-neutral-800 px-2 py-0.5 text-xs text-neutral-500">{{ $category->posts_count ?? 0 }}</span></a></li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="fade-up card-glass rounded-xl border border-neutral-800 bg-neutral-900/60 backdrop-blur-sm p-6">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2">
                            @foreach($tags ?? [] as $tag)
                                <a href="{{ route('blog.tag', $tag->slug) }}" class="tag {{ request('tag') === $tag->slug ? '!bg-primary-600 !text-white' : '' }}">{{ $tag->name }}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
