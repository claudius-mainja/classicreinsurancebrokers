...
<div x-data="{ scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = (window.scrollY / (document.documentElement.scrollHeight - window.innerHeight)) * 100 })" class="relative">
    ...
    <section class="hero-gradient">
        <div class="aspect-[21/9] overflow-hidden sm:aspect-[3/1] lg:aspect-[4/1]">
            <img src="{{ asset($post->featured_image ?? 'images/blog-placeholder.jpg') }}" alt="{{ $post->title }}" class="h-full w-full object-cover opacity-50">
        </div>
            ...
            <div class="mx-auto max-w-4xl px-6 pb-12 sm:pb-16 lg:px-8">
                <div class="fade-up">
                    <div class="mb-4 flex flex-wrap items-center gap-3">
                        <span class="inline-flex items-center rounded-full bg-primary-500/20 px-3 py-1 text-xs font-medium text-primary-200">{{ $post->category->name ?? 'Insurance' }}</span>
    ...
    <div class="mx-auto max-w-7xl px-6 py-12 lg:px-8">
        <div class="grid gap-12 lg:grid-cols-[1fr_300px]">
            <article class="prose prose-lg max-w-none prose-headings:font-heading prose-headings:text-neutral-900 prose-a:text-primary-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl">
                {!! $post->content !!}
                ...
                <div class="not-prose mt-10 card p-8">
                    ...
                    <form action="{{ route('blog.comment', $post->slug) }}" method="POST" class="mt-6 space-y-4">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div><label for="comment-name" class="block text-sm font-medium text-neutral-700">Name</label><input type="text" id="comment-name" name="name" required class="input-field mt-1"></div>
                            <div><label for="comment-email" class="block text-sm font-medium text-neutral-700">Email</label><input type="email" id="comment-email" name="email" required class="input-field mt-1"></div>
                        </div>
                        <div><label for="comment-message" class="block text-sm font-medium text-neutral-700">Comment</label><textarea id="comment-message" name="message" rows="4" required class="input-field mt-1 resize-none"></textarea></div>
            ...
            <aside class="lg:border-l lg:border-neutral-100 lg:pl-12">
                <div class="sticky top-24 space-y-6">
                    <div class="fade-up card p-6 text-center">
                        <h3 class="text-base font-bold text-neutral-900">Subscribe</h3>
                        ...
                        <form action="{{ route('newsletter.subscribe') }}" method="POST" class="mt-4">
                            @csrf
                            <input type="email" name="email" placeholder="your@email.com" required class="input-field">
                            <button type="submit" class="btn btn-primary mt-3 w-full text-sm">Subscribe</button>
                        </form>
                    </div>
                    @if($relatedPosts->count() > 0)
                        <div class="fade-up">
                            <h3 class="mb-4 text-sm font-semibold uppercase tracking-widest text-neutral-900">Related articles</h3>
                            <div class="space-y-4">
                                @foreach($relatedPosts as $related)
                                    <a href="{{ route('blog.show', $related->slug) }}" class="group flex gap-4">
                                        <div class="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-neutral-100"><img src="{{ asset($related->featured_image ?? 'images/blog-placeholder.jpg') }}" alt="{{ $related->title }}" class="h-full w-full object-cover"></div>
                                        <div><h4 class="text-sm font-semibold text-neutral-900 transition-colors group-hover:text-primary-600 line-clamp-2">{{ $related->title }}</h4><p class="mt-1 text-xs text-neutral-400">{{ $related->published_at->format('M d, Y') }}</p></div>
                                    </a>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="fade-up hero-gradient card !border-0 p-6 text-center">
                        ...
                        <a href="{{ route('contact') }}" class="btn mt-4 inline-flex w-full items-center justify-center bg-white text-primary-700 hover:bg-primary-50 text-sm">Contact us</a>
                    </div>
                </div>
            </aside>
        </div>
